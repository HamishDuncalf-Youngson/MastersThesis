...
Leaf <- ggplot(leafsum, aes(x= Location, y= leafk, fill=Location))+
  ...
  labs (x= "Location",  y= expression(paste("Decay constant k " (DD^-1))))  + scale_fill_manual(values = c('turquoise','deepskyblue4', 'royalblue1')) +
  facet_wrap(~Mesh)
...
leaffineddsum<- summarySE(F,measurevar= "fineleafddklog1", groupvars=c("Location"),na.rm = TRUE)
...
degreedaysteabag<- ggplot(teabagddsum2, aes(x= Location, y=teabagddk, fill=Location)) +
  geom_bar(position = "dodge", stat = "identity")+
  labs(title = "Teabag Decomposition (Temperature Corrected") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
...
wettexcoarseddsum<- summarySE(F,measurevar= "wettexcddklog", groupvars=c("Location"),na.rm = TRUE)
wettexcoarseddsum2 <- wettexcoarseddsum

wettexfineddsum<- summarySE(F,measurevar= "wettexfddklog", groupvars=c("Location"),na.rm = TRUE)
...
wettexddsum$klog <- wettexddsum$wettexk*-1


wettexddsum$Location <- factor(wettexddsum$Location)
wettexdd <- ggplot(wettexddsum, aes(x= Location, y= klog, fill=Location))+
  geom_bar(position = "dodge", stat = "identity")+
  labs(title = "Wettex Decomposition (Temperature Corrected)") + 
  theme(panel.background = element_rect(fill = NA),  
        panel.grid.major = element_line(colour = "white"), axis.line = element_line(size = 0.5, colour = "grey"),
        axis.ticks = element_line(size = 0.5)) +
  geom_errorbar(aes(ymin= klog-se, ymax= klog+se),  width=.22, position=position_dodge(.9)) +
  labs (x= "Location",  y="Decay constant k log  (DD-1)")  + scale_fill_manual(values = c('turquoise','deepskyblue4', 'royalblue1')) +
  facet_wrap(~Mesh)
...
grid.arrange(Imidaclopridlocation,Fipronillocation, ncol=2, nrow = 1)
...
leafcoarseddsumriv<- summarySE(F,measurevar= "coarseleafddk1", groupvars=c("River.Reach"),na.rm = TRUE)
...
leafcoarseddsumriv2$River.Reach <- factor(leafcoarseddsumriv2$River.Reach)
degreedayscoarseleafriv <- ggplot(leafcoarseddsumriv2, aes(x= River.Reach, y=coarseleafddk1, fill=River.Reach)) +
  geom_bar(position = "dodge", stat = "identity")+
  ...
  labs (x= "River/Reach",  y="Decay constant (kdd)")  +  scale_color_futurama() + facet_wrap(~Location, scales="free")
